...
#'
#' @param x National Accounts File for Variable Production, Exports, Value added and Hours worked as R data table
#' @param y EGSS-Data set created with Function loadEGSS()
#' @param toEst Year to be predicted
#' @param t1 Type variable for which the analysis should be performed. Possible values are TOT_EGSS (default),
#'           MKT, NMKT, ANC, C_REP, ES_CS, OWN_USE and REST
#' @return Complete EGSS-Data Matrix enriched with National Account Data and respective ratios of
...
#' @import data.table
#' @export
loadNA <- function(x, y, toEst, t1 = "TOT_EGSS"){
  ...
  x <- x[time <= toEst,]
  x[,":=" (yyyy = time, time = NULL)]
  #--------------------------------------------------------------------------

  # distribute the National Accounts Aggregate E37-E39 to each E37, E38 and E39
  n37 <- x[nace_r2=="E37-E39",]
  n37[,":=" (nace_r2="E37")]
  n38 <- x[nace_r2=="E37-E39",]
  n38[,":=" (nace_r2="E38")]
  n39 <- x[nace_r2=="E37-E39",]
  n39[,":=" (nace_r2="E39")]
  x <- x[!nace_r2=="E37-E39",]
  x <- do.call("rbind", list(x, n37, n38, n39))
  #----------------------------------------------------------------------------
  ...
  setkey(x,geo,yyyy,indic_pi,nace_r2)
  y3 <- merge(y2,x,all.x=TRUE)
  ...
  y3[,":=" (code=paste0(geo,"_",z0,"_",z3,"_",z1,"_",z2),z0=NULL,z1=NULL,z2=NULL,z3=NULL)]
  y3[,":=" (lcode=substr(code,1,6))]
  # ........................................................................ End Coding (1)

  # Calculation of relative observation values: relative observation values for each code are based
  # on the lowest continuous (for all years) available aggregation level for that very code
  # from national account data.
  naRef <- y3[, lapply(.SD,function(x){
     #all(!is.na(x))
     all(x>0)
    }),by=c("code"),.SDcols=c("naBASE","na1DIG","naINDIC","naGEO")]
  naRef[,":=" (naBASE_av=naBASE,na1DIG_av=na1DIG,naINDIC_av=naINDIC,naGEO_av=naGEO,
               naBASE=NULL,na1DIG=NULL,naINDIC=NULL,naGEO=NULL),]
  setkey(naRef,code)
  setkey(y3,code)
  ...
  y4 <- y3a[nace_r2 %in% c(LETTERS[1:21],"TOTAL") & ty==t1 & ceparema %in% c("CEPA1", "CEPA2", "CEPA3", "CEPA4",
                                                                                  "CEPA5", "CEPA6", "CEPA7-9", "CREMA10",
                                                                                  "CREMA11", "CREMA12_15_16", "CREMA13",
                                                                                  "CREMA14","TOT_CEPA","TOT_CREMA","TOTAL"),]
  y4 <- y4[,.(code, geo, yyyy, indic_pi, nace_r2, ceparema, ty, unit, obs_value, rev_val, obs_value_rel, obs_status,
              obs_conf, obs_comment, obs_gen, orig, lcode, naINDIC)]
  return(y4[])
  }
